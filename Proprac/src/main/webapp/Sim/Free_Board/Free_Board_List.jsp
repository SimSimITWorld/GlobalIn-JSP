		...
		<div class="board_write_wrap">	<!-- 타이틀 아래 영역(리스트, 페이지, 버튼) -->
			...
			<c:if test="${count>0 }">
				...
				<c:if test="${startPage>pageBlock }">
					<a href="/Proprac/Sim/Free_Board/Free_Board.do?free=list&pageNo=${startPage-pageBlock+2 }" onclick="frm_sub(${startPage-pageBlock})" class="bt prev"><</a>
				</c:if>
				
				<c:forEach var="i" begin="${startPage }" end="${endPage }">
					<a href="/Proprac/Sim/Free_Board/Free_Board.do?free=list&pageNo=${i }" onclick="frm_sub(${i })" class="num on">${i }</a> 
				</c:forEach>
				
				<c:if test="${endPage<pageCount }">
					<a href="/Proprac/Sim/Free_Board/Free_Board.do?free=list&pageNo=${startPage+pageBlock }" onclick="frm_sub(${startPage+pageBlock})" class="bt next">></a>
				</c:if>
			</c:if>
			</div>
